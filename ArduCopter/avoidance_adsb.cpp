...
bool AP_Avoidance_Copter::handle_avoidance_vertical(const AP_Avoidance::Obstacle *obstacle, bool allow_mode_change)
{
    ...
    if (should_climb) {
        velocity_neu.z = copter.wp_nav->get_default_speed_up() * 100.0f; // convert m to cm
    } else {
        velocity_neu.z = -copter.wp_nav->get_default_speed_down() * 100.0f; // convert m to cm
...
bool AP_Avoidance_Copter::handle_avoidance_horizontal(const AP_Avoidance::Obstacle *obstacle, bool allow_mode_change)
{
    ...
    if (get_vector_perpendicular(obstacle, velocity_neu)) {
        ...
        velocity_neu.x *= copter.wp_nav->get_default_speed_xy() * 100.0f; // convert m to cm
        velocity_neu.y *= copter.wp_nav->get_default_speed_xy() * 100.0f; // convert m to cm
        // send target velocity
        copter.mode_avoid_adsb.set_velocity(velocity_neu);
        return true;
    }
