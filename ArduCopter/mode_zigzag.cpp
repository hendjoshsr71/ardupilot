...
void ModeZigZag::save_or_move_to_destination(Destination ab_dest)
{
    ...
    const Vector2f curr_pos {inertial_nav.get_position_xy() * 100.0};
...
bool ModeZigZag::calculate_next_dest(Destination ab_dest, bool use_wpnav_alt, Vector3f& next_dest, bool& terrain_alt) const
{
    ...
    const Vector2f curr_pos2d {inertial_nav.get_position_xy() * 100.0};
    ...
    if (use_wpnav_alt) {
        ...
        if (terrain_alt) {
            ...
            next_dest.z = pos_control->is_active_z() ? pos_control->get_pos_target_z_cm() : inertial_nav.get_position_z_down() * 100.0;
        }
    }
