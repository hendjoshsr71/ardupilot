...
void ModeZigZag::return_to_manual_control(bool maintain_target)
{
    if (stage == AUTO) {
        ...
        loiter_nav->clear_pilot_desired_acceleration();
        if (maintain_target) {
            const Vector3f& wp_dest = wp_nav->get_wp_destination() * 100.0f; // convert m to cm
            loiter_nav->init_target(wp_dest.neu_to_ned());
...
bool ModeZigZag::reached_destination()
{
    ...
    if ((wp_nav->get_wp_distance_to_destination() * 100.0f) > ZIGZAG_WP_RADIUS_CM) { // convert m to cm
        return false;
    }
...
bool ModeZigZag::calculate_next_dest(Destination ab_dest, bool use_wpnav_alt, Vector3f& next_dest, bool& terrain_alt) const
{
    ...
    if (use_wpnav_alt) {
        ...
        next_dest.z = wp_nav->get_wp_destination().neu_to_ned().z * 100.0f;  // convert m to cm
    } else {
