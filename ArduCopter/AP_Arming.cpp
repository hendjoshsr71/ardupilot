...
bool AP_Arming_Copter::barometer_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_BARO)) {
        ...
        if (using_baro_ref) {
            if (fabsf(copter.inertial_nav.get_position_z_down() * 100.0 - copter.baro_alt) > PREARM_MAX_ALT_DISPARITY_CM) {
...
bool AP_Arming_Copter::arm(const AP_Arming::Method method, const bool do_arming_checks)
{
    ...
    if (!ahrs.home_is_set()) {
        ...
        copter.arming_altitude_m = copter.inertial_nav.get_position_z_down();
    }
