...
uint32_t ModeAuto::wp_distance() const
{
    ...
    case SubMode::CIRCLE_MOVE_TO_EDGE:
    default:
        return wp_nav->get_wp_distance_to_destination() * 100.0f; // convert meters to cm
    }
}
...
void ModeAuto::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (target_loc.lat == 0 && target_loc.lng == 0) {
        ...
        const Location temp_loc(temp_pos * 100.0f, Location::AltFrame::ABOVE_ORIGIN); // convert meters -> cm
    ...
    switch (state) {
        case State::FlyToLocation:
            ...
            if (copter.wp_nav->reached_wp_destination()) {
                ...
                const Vector3f& dest = copter.wp_nav->get_wp_destination().neu_to_ned() * 100.0f; // convert m to cm
