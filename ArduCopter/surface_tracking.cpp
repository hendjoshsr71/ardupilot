...
void Copter::SurfaceTracking::update_surface_offset()
{
    ...
    if (((surface == Surface::GROUND) && copter.rangefinder_alt_ok() && (copter.rangefinder_state.glitch_count == 0)) ||
        ...
        const float curr_surface_alt_above_origin_cm = (copter.inertial_nav.get_position_z_down() * 100.0) - dir * rf_state.alt_cm;
...
void Copter::SurfaceTracking::set_target_alt_cm(float _target_alt_cm)
{
    ...
    copter.pos_control->set_pos_offset_z_cm((copter.inertial_nav.get_position_z_down() * 100.0) - _target_alt_cm);
