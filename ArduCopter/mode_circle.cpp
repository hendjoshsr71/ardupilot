...
void ModeCircle::run()
{
    ...
    if (!copter.failsafe.radio && copter.circle_nav->pilot_control_enabled()) {
        ...
        const float nav_speed = copter.wp_nav->get_default_speed_xy() * 100.0;          // copter WP_NAV parameter speed,  // convert m to cm
    ...
    copter.failsafe_terrain_set_status(copter.circle_nav->update(target_climb_rate * 0.01)); // convert cm to meters
