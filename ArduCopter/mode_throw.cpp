...
bool ModeThrow::init(bool ignore_checks)
{
    ...
    pos_control->set_max_speed_accel_xy(wp_nav->get_default_speed_xy() * 100.0, BRAKE_MODE_DECEL_RATE); // convert m to cm
    pos_control->set_correction_speed_accel_xy(wp_nav->get_default_speed_xy() * 100.0, BRAKE_MODE_DECEL_RATE);  // convert m to cm
