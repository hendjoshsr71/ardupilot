...
bool ModeGuided::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
    ...
    guided_pos_target_cm = destination.topostype().neu_to_ned();
...
void ModeGuided::angle_control_run()
{
    ...
    if (!guided_angle_state.use_thrust) {
        ...
        climb_rate_cms = constrain_float(guided_angle_state.climb_rate_cms, -fabsf(wp_nav->get_default_speed_down()) * 100.0, wp_nav->get_default_speed_up() * 100.0);  // convert m to cm
...
void ModeGuided::limit_init_time_and_pos()
{
    ...
    guided_limit.start_pos = inertial_nav.get_position(); // xy only used
}
...
bool ModeGuided::limit_check()
{
    ...
    if (guided_limit.horiz_max_cm > 0.0f) {
        float horiz_move = get_horizontal_distance_cm(guided_limit.start_pos, curr_pos); // only xy only
