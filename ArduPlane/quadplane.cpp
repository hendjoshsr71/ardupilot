...
float QuadPlane::landing_descent_rate_cms(float height_above_ground)
{
    ...
    const float max_climb_speed = wp_nav->get_default_speed_up() * 100.0; // convert m to cm
    float ret = linear_interpolate(land_speed_cms, wp_nav->get_default_speed_down() * 100.0,  // convert m to cm
                                   height_above_ground,
...
float QuadPlane::get_pilot_input_yaw_rate_cds(void) const
{
    ...
    if (!manual_air_mode &&
        ...
        plane.arming.get_rudder_arming_type() != AP_Arming::RudderArming::IS_DISABLED && !(inertial_nav.get_climb_rate() < -0.5 * get_pilot_velocity_z_max_dn())) {
...
float QuadPlane::get_desired_yaw_rate_cds(void)
{
    ...
    bool manual_air_mode = plane.control_mode->is_vtol_man_throttle() && air_mode_active();
    if (plane.get_throttle_input() <= 0 && !plane.control_mode->does_auto_throttle() && !manual_air_mode && !(inertial_nav.get_climb_rate() < -0.5 * get_pilot_velocity_z_max_dn())) {
        // the user may be trying to disarm
        return 0;
    }
    // add in pilot input
...
float QuadPlane::assist_climb_rate_cms(void) const
{
    ...
    climb_rate = constrain_float(climb_rate, -wp_nav->get_default_speed_down() * 100.0, wp_nav->get_default_speed_up() * 100.0); // convert m to cm
...
void QuadPlane::update_throttle_suppression(void)
{
    ...
    if (fabsf(inertial_nav.get_climb_rate()) > 100) {
        return;
    }
...
void QuadPlane::update_throttle_hover()
{
    ...
    if (throttle > 0.0f && fabsf(inertial_nav.get_climb_rate()) < 60 &&
...
void QuadPlane::update_land_positioning(void)
{
    ...
    const float speed_max_cms = wp_nav->get_wp_acceleration() * 100.0 * 0.5; // convert m to cm
...
void QuadPlane::run_xy_controller(void)
{
    if (!pos_control->is_active_xy()) {
        ...
        pos_control->set_correction_speed_accel_xy(wp_nav->get_default_speed_xy() * 100.0, wp_nav->get_wp_acceleration() * 100.0); // convert m to cm
...
void QuadPlane::PosControlState::set_state(enum position_control_state s)
{
    if (state != s) {
        ...
        if (s == QPOS_POSITION1) {
            ...
            // back to normal
            qp.pos_control->set_max_speed_accel_xy(qp.wp_nav->get_default_speed_xy() * 100.0,
                                                   qp.wp_nav->get_wp_acceleration() * 100.0); // convert m to cm
            qp.pos_control->set_correction_speed_accel_xy(qp.wp_nav->get_default_speed_xy() * 100.0,
                                                   qp.wp_nav->get_wp_acceleration() * 100.0); // convert m to cm
...
void QuadPlane::vtol_position_controller(void)
{
    ...
    case QPOS_LAND_DESCEND: {
        ...
        pos_control->set_max_speed_accel_xy(scaled_wp_speed * 100, wp_nav->get_wp_acceleration() * 100.0); // convert m to cm
        pos_control->set_correction_speed_accel_xy(scaled_wp_speed * 100, wp_nav->get_wp_acceleration() * 100.0);  // convert m to cm
...
float QuadPlane::get_scaled_wp_speed(float target_bearing_deg) const
{
    ...
    const float wp_speed = wp_nav->get_default_speed_xy();
...
void QuadPlane::takeoff_controller(void)
{
    ...
    */
    ...
    set_climb_rate_cms(wp_nav->get_default_speed_up() * 100.0, false);  // convert m to cm
    run_z_controller();
}
...
bool QuadPlane::do_vtol_takeoff(const AP_Mission::Mission_Command& cmd)
{
    ...
    const float vel_z = inertial_nav.get_climb_rate() * 0.01f;
...
bool QuadPlane::verify_vtol_land(void)
{
    ...
    if (poscontrol.get_state() == QPOS_POSITION2) {
        ...
        if (poscontrol.pilot_correction_done) {
            ...
            const float dist = (inertial_nav.get_position().neu_to_ned().topostype() - poscontrol.target_cm).xy().length() * 0.01; // z not used here
...
void QuadPlane::Log_Write_QControl_Tuning()
{
    ...
    struct log_QControl_Tuning pkt = {
        ...
        climb_rate          : int16_t(inertial_nav.get_climb_rate()),
...
int8_t QuadPlane::forward_throttle_pct()
{
    ...
    */
    ...
    // WP nav speed.
    fwd_vel_error -= wp_nav->get_default_speed_xy() * plane.nav_pitch_cd / (float)plane.aparm.pitch_limit_max_cd;
