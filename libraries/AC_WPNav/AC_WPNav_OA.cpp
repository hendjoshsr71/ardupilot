...
float AC_WPNav_OA::get_wp_distance_to_destination() const
{
    ...
    const Vector3f &curr = _inav.get_position() * 0.01f; // convert cm to meters, _inav returns neu but z not used here
...
bool AC_WPNav_OA::update_wpnav()
{
    ...
    if ((oa_ptr != nullptr) && AP::ahrs().get_position(current_loc)) {
        ...
        const Location origin_loc(_origin_oabak.neu_to_ned() * 100.0f, _terrain_alt_oabak ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);                // I didnt have neu to ned and it worked ???
        const Location destination_loc(_destination_oabak.neu_to_ned() * 100.0f, _terrain_alt_oabak ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);      // I didnt have neu to ned and it worked ???
        ...
        case AP_OAPathPlanner::OA_ERROR:
            ...
            if ((_oa_state != AP_OAPathPlanner::OA_PROCESSING) && (_oa_state != AP_OAPathPlanner::OA_ERROR)) {
                ...
                _oa_destination = Location(stopping_point.neu_to_ned() * 100.0f, Location::AltFrame::ABOVE_ORIGIN); // convert meters to cm
        ...
        case AP_OAPathPlanner::OA_SUCCESS:
            ...
            if ((_oa_state != AP_OAPathPlanner::OA_SUCCESS) || !oa_destination_new.same_latlon_as(_oa_destination)) {
                ...
                if (_oa_destination.get_vector_from_origin_NED(dest_NED)) {
                    ...
                    if (set_wp_destination(dest_NED, _terrain_alt_oabak)) {
                        _oa_state = oa_retstate;
                    }
                }
            }
