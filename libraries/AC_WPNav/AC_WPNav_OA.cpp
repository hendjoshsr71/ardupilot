...
/// set_origin_and_destination - set origin and destination waypoints using position vector (distance from ekf origin frame NED in cm)
    ...
    if ((oa_ptr != nullptr) && AP::ahrs().get_position(current_loc)) {
        ...
        case AP_OAPathPlanner::OA_NOT_REQUIRED:
            if (_oa_state != oa_retstate) {
                ...
                set_wp_destination( _destination_oabak * 0.01f, _terrain_alt); // convert from cm to m
        ...
        case AP_OAPathPlanner::OA_ERROR:
            ...
            if ((_oa_state != AP_OAPathPlanner::OA_PROCESSING) && (_oa_state != AP_OAPathPlanner::OA_ERROR)) {
                ...
                if (set_wp_destination(stopping_point * 0.01f, false)) { // convert from cm to m
                    _oa_state = oa_retstate;
                }
            }
            break;
        case AP_OAPathPlanner::OA_SUCCESS:
            ...
            if ((_oa_state != AP_OAPathPlanner::OA_SUCCESS) || !oa_destination_new.same_latlon_as(_oa_destination)) {
                ...
                if (_oa_destination.get_vector_from_origin_NED_cm(dest_NED)) {
                    ...
                    if (set_wp_destination(dest_NED * 0.01f, _terrain_alt)) { // convert from cm to m
                        _oa_state = oa_retstate;
                    }
                }
            }
