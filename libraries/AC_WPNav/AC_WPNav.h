...
public:
    ...
    float get_default_speed_xy() const { return _wp_speed_cms * 0.01f; }

    /// get default target climb speed in m/s during missions
    float get_default_speed_up() const { return _wp_speed_up_cms * 0.01f; }

    /// get default target descent rate in m/s during missions. Note: always positive
    float get_default_speed_down() const { return _wp_speed_down_cms * 0.01f; }

    /// get_accel_z - returns target descent speed in m/s during missions. Note: always positive
    float get_accel_z() const { return _wp_accel_z_cmss * 0.01f; }

    /// get_wp_acceleration - returns acceleration in m/s/s during missions
    float get_wp_acceleration() const { return _wp_accel_cmss.get() * 0.01f; }
    ...
    bool reached_wp_destination_xy() const {
        return get_wp_distance_to_destination() < _wp_radius_cm * 0.01f;
    }
...
protected:
    ...
    // position, velocity and acceleration targets passed to position controller, defined with z-axis up
