...
void AC_Circle::init()
{
    ...
    _radius = _radius_parm * 0.01f; // convert user params in cm to meters
    _last_radius_param = _radius_parm * 0.01f; // convert user params in cm to meters
    ...
    // get stopping point
    const Vector3f& stopping_point = _pos_control.get_pos_target_cm() * 0.01f; // convert cm to meters
...
void AC_Circle::set_center(const Location& center)
{
    if (center.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) {
        ...
        if (center.get_vector_xy_from_origin_NE(center_xy) && center.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, terr_alt_cm)) {
            set_center(Vector3f(center_xy.x, center_xy.y, -terr_alt_cm) * 0.01f, true); // convert altitude to NED, convert cm to meters
        } else {
...
/// set_radius - set circle radius in meters
void AC_Circle::set_radius(float radius)
{
    _radius = constrain_float(radius, 0, AC_CIRCLE_RADIUS_MAX);
}
...
// climb_rate in m/s
bool AC_Circle::update(float climb_rate)
{
    ...
    float target_z;
    if (_terrain_alt) {
        target_z = _center.z - terr_offset; // Convert terr_offset NEU to NED
    } else {
        target_z = -_pos_control.get_pos_target_z_cm() * 0.01f; // Convert NEU to NED, convert cm to meters
    }
    ...
    Vector3f zero;
    target = target.neu_to_ned() * 100.0f; // convert meters to cm
    _pos_control.input_pos_vel_accel_xy(target, zero, zero); // pos_control uses NEU
    if(_terrain_alt) {
        _pos_control.input_pos_vel_accel_z(target, zero, zero); // pos_control uses NEU
    } else {
        _pos_control.set_pos_target_z_from_climb_rate_cm(climb_rate * 100.0f,  false);
    }
...
// get_closest_point_on_circle - returns closest point on the circle frame NED in meters
...
void AC_Circle::calc_velocities(bool init_velocity)
{
    ...
    if (_radius <= 0) {
        ...
        float velocity_max = MIN(_pos_control.get_max_speed_xy_cms() * 0.01f, safe_sqrt(0.5f*_pos_control.get_max_accel_xy_cmss() * 0.01f * _radius)); // convert cm to meters
        ...
        // angular_velocity in radians per second
        _angular_accel = MAX(_pos_control.get_max_accel_xy_cmss() * 0.01f /_radius, ToRad(AC_CIRCLE_ANGULAR_ACCEL_MIN));
    }
...
bool AC_Circle::get_terrain_offset(float& offset)
{
    ...
    case AC_Circle::TerrainSource::TERRAIN_FROM_RANGEFINDER:
        if (_rangefinder_healthy) {
            offset = (_inav.get_altitude() * 0.01f) - _rangefinder_alt;
            return true;
        }
        return false;
    case AC_Circle::TerrainSource::TERRAIN_FROM_TERRAINDATABASE:
        ...
        if (terrain != nullptr && terrain->height_above_terrain(terr_alt, true)) {
            offset = (_inav.get_altitude() * 0.01f) - terr_alt;
            return true;
        }
#endif
        return false;
...
void AC_Circle::check_param_change()
{
    if (!is_equal(_last_radius_param, _radius_parm.get() * 0.01f)) {
        _radius = _radius_parm * 0.01f;
        _last_radius_param = _radius_parm * 0.01f;
    }
}
