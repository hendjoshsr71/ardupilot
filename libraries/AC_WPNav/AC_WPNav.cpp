...
void AC_WPNav::shift_wp_origin_and_destination_to_current_pos_xy()
{
    ...
    const Vector3f& curr_pos = _inav.get_position() * 0.01;  // inav returns neu but z not used here, convert cm to m
...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    if (is_positive(curr_target_vel.length())) {
        ...
        const float track_velocity = (_inav.get_velocity() * 0.01) * track_direction; // convert cm to m
...
float AC_WPNav::get_wp_distance_to_destination() const
{
    ...
    const Vector3f &curr = _inav.get_position() * 0.01;    // cm -> meters
...
{
    return get_bearing_cd(_inav.get_position() * 0.01, _destination);  // convert cm to m
}
