...
uint8_t AP_BattMonitor_SMBus::read_name(AP_BattMonitor_SMBus::BATTMONITOR_SMBUS reg_name, char * name_out, uint8_t buflen)
{
    ...
    if (reg_name == AP_BattMonitor_SMBus::BATTMONITOR_SMBUS::BATTMONITOR_SMBUS_MANUFACTURE_NAME ||
        ...
        uint8_t name[AP_BATTMONITOR_SMBUS_READ_BLOCK_MAXIMUM_TRANSFER + 1];
        const uint8_t name_len = read_block(reg_name, name, AP_BATTMONITOR_SMBUS_READ_BLOCK_MAXIMUM_TRANSFER);
...
#endif // AP_BATTERY_SMART_BATTERY_INFO_ENABLED
