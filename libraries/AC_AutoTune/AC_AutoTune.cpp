...
void AC_AutoTune::get_poshold_attitude(float &roll_cd_out, float &pitch_cd_out, float &yaw_cd_out)
{
    ...
    if (!have_position) {
        have_position = true;
        start_position = inertial_nav->get_position_ned().neu_to_ned() * 100.0; // convert m to cm
    }
    ...
    Vector3f pdiff = (inertial_nav->get_position_ned().neu_to_ned() * 100.0) - start_position; // convert m to cm
    pdiff.z = 0;
