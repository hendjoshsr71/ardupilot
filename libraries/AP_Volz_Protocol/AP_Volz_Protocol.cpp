...
void AP_Volz_Protocol::update()
{
    ...
    // for (uint8_t i=0; i<NUM_SERVO_CHANNELS; i++) {
        ...
        case Protocol::DA26_RS485:
            {
                const float angle =  compute_angle_from_pwm(last_updated_channel, output_pwm, DA26_POSITION_ANGLE_MIN, DA26_POSITION_ANGLE_MAX);
                cmd_transmit = da26_compute_cmd_to_tx(angle);
                break;
            }
        case Protocol::UAVOS_VOLZ_RS485_ICD:
            {
                const float angle =  compute_angle_from_pwm(last_updated_channel, output_pwm, UAVOS_VOLZ_RS485_ICD_POSITION_ANGLE_MIN, UAVOS_VOLZ_RS485_ICD_POSITION_ANGLE_MIN);
