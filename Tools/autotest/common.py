...
#            self.remove_bin_logs() # can't do this as one of the binlogs is probably open for writing by the SITL process.  If we force a rotate before running tests then we can do this.  # noqa
    ...
    def test_set_position_global_int(self, timeout=100):
        ...
        # set the start location to the Grand Cayon for quick slopping terrain
        self.customise_SITL_commandline(["--home", "%s,%s,%s,%s"
                                         % (36.011874, -113.810935, 1446.0, 90.0)])
        mavproxy = self.start_mavproxy()
        ...
        def to_alt_frame(loc, mav_frame):
            if mav_frame in ["MAV_FRAME_GLOBAL_RELATIVE_ALT",
                ...
                # terrain_alt = terrain.current_height
        ...
        def check_position_target_msg(rcvd_type_mask, frame, targetpos=None, vel=None, accel=None, yaw=None, yaw_rate=None):
            ...
            if not (rcvd_type_mask & MAV_POS_TARGET_TYPE_MASK.POS_IGNORE):
                ...
                alt_converted = to_alt_frame(targetpos, frame_name)
        ...
        for frame in MAV_FRAMES_TO_TEST:
            ...
            rcvd_type_mask = send_target_position(targetpos.lat, targetpos.lng,
                                                  to_alt_frame(targetpos, frame_name), frame)
            ...
            if test_alt:
                targetpos.alt -= 50
            rcvd_type_mask = send_target_position(targetpos.lat, targetpos.lng, to_alt_frame(targetpos, frame_name), frame)
            ...
            if test_heading:
                ...
                self.mav.mav.set_position_target_global_int_send(
                    ...
                    to_alt_frame(targetpos, frame_name),  # alt
                    0,  # vx
                    0,  # vy
                    0,  # vz
                    0,  # afx
