...
MAV_RESULT GCS_MAVLINK_Sub::handle_command_long_packet(const mavlink_command_long_t &packet)
{
    ...
    case MAV_CMD_DO_CHANGE_SPEED:
        ...
        if (packet.param2 > 0.0f) {
            sub.wp_nav.set_speed_xy(packet.param2);
            return MAV_RESULT_ACCEPTED;
        }
        return MAV_RESULT_FAILED;
...
void GCS_MAVLINK_Sub::handleMessage(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_SET_ATTITUDE_TARGET: { // MAV ID: 82
        ...
        if (is_equal(packet.thrust, 0.5f)) {
            ...
            climb_rate_cms = (packet.thrust - 0.5f) * 2.0f * sub.wp_nav.get_default_speed_up() * 100.0f;  // convert m to cm
        } else {
            // descend at up to WPNAV_SPEED_DN
            climb_rate_cms = (packet.thrust - 0.5f) * 2.0f * fabsf(sub.wp_nav.get_default_speed_down() * 100.0f); // convert m to cm
        }
