...
void Sub::auto_circle_movetoedge_start(const Location &circle_center, float radius_m)
{
    ...
    float dist_to_edge = ((inertial_nav.get_position_ned().neu_to_ned() * 100.0) - circle_edge_neu).length();
    ...
    if (dist_to_edge > 300.0f) {
        ...
        float dist_to_center = get_horizontal_distance_cm(inertial_nav.get_position_xy().topostype() * 100.0, circle_nav.get_center().xy());
